...
const Select = ({ data, label, placeholder, block = false, invalid = false, required = false, disabled = false, wrapperProps, ...props }) => {
    ...
    return (
        <Wrapper block={block} {...wrapperProps}>
            <Label>{label}</Label>
            <StyleSelect invalid={invalid} required={required} disabled={disabled} {...props}>{options}</StyleSelect>
        </Wrapper>
    )
}
